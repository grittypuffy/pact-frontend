...
const StatisticsComparison: React.FC<StatisticsComparisonProps> = ({
    ...
    const [optimized, setOptimized] = useState<PromptStatistics>(optimizedStats);
    ...
    return (
        <div className="w-full max-w-6xl mx-auto p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {metrics.map((metric) => {
                    console.log("original", original[metric.key]);
                    const originalValue = (original[metric.key] || 0) * 10;
                    const optimizedValue = (optimized[metric.key] || 0) * 10;
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex items-center space-x-2">
                                {/* Original Value */}
                                <div className="w-1/2">
                                    <div className="text-xs mb-1" style={{ color: "rgba(var(--text-color), 0.7)" }}>
                                        Original
                                    </div>
                                    <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
                                        <div
                                            className={`h-2.5 rounded-full ${getMetricColor(originalValue)}`}
                                            style={{ width: `${originalValue}%` }}
                                        ></div>
                                    </div>
                                    <div className="text-xs text-right mt-1" style={{ color: "rgba(var(--text-color), 0.9)" }}>
                                        {originalValue}%
                                    </div>
                                </div>
                                ...
                                <div className="w-1/2">
                                    ...
                                    <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
                                        <div
                                            className={`h-2.5 rounded-full ${getMetricColor(optimizedValue)}`}
